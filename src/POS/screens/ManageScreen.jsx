...
function createEmptyProductDraft(categoryId = '') {
  return {
    name: '',
    category: categoryId,
    label: 'Coffee',
          ...
          {options.map((option) => {
            ...
            return (
              <button
                ...
                className={`flex w-full cursor-pointer select-none items-center justify-between rounded-lg px-3 py-2 text-sm font-semibold transition-colors disabled:cursor-not-allowed disabled:opacity-70 ${isActive
...
export function ManageScreen({
  ...
  useEffect(() => {
    ...
    setSettings((previous) => ({
      ...
      currency: String(settingsBootstrap.currency ?? previous.currency ?? 'USD'),
      exchangeRate: Number(settingsBootstrap.exchangeRate ?? previous.exchangeRate ?? 4000),
    }))
  }, [settingsBootstrap])
    ...
    fetchSettings(controller.signal)
      .then((result) => {
        setSettings((previous) => ({
          ...
          currency: String(result?.currency ?? previous.currency ?? 'USD'),
          exchangeRate: Number(result?.exchangeRate ?? previous.exchangeRate ?? 4000),
        }))
      })
      .catch(() => { })
          ...
          <section className="ui-surface-muted p-4 md:p-6">
                ...
                {alertProducts.length > 0 && (
                  <div className="mt-4 rounded-lg border border-amber-200 bg-amber-50 px-4 py-2">
                    <p className="text-sm font-semibold text-amber-800">
                      {alertProducts.length} items need attention (low/out of stock)
                    </p>
                  </div>
                )}
                        ...
                        <div className="inline-flex rounded-lg border border-slate-200 bg-white p-0.5">
                          {[
                            ...
                            <button
                              ...
                              className={`ui-btn rounded-md px-2.5 py-1 text-xs transition-colors ${inventoryFilter === option.id
                          ...
                          return (
                            ...
                            >
                              ...
                              <div className="flex justify-center">
                                <span className={`rounded-md px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider border ${isOut ? 'bg-rose-50 text-rose-600 border-rose-100' :
            ...
            {page === 'teams' && (
              <div className="space-y-4">
                <div className="rounded-xl border border-slate-200 bg-white p-4">
                  ...
                  {canCreateUsers && (
                    <div className="mb-4 grid gap-2 md:grid-cols-[1fr_1fr_120px_120px_auto]">
                      <input
                  ...
                  <div className="overflow-x-auto">
                    <div className="max-h-[400px] min-w-[620px] overflow-y-auto rounded-lg border border-slate-100">
                      <div className="grid grid-cols-[60px_1.2fr_1.5fr_110px_100px] gap-3 border-b border-slate-100 bg-slate-50 px-4 py-3 text-xs font-semibold uppercase tracking-wide text-slate-500 sticky top-0">
                        <p>ID</p>
                        <p>Username</p>
                        <p>Name</p>
                        <p>Role</p>
                      ...
                      {users.map((user) => (
                        <div
                          key={user.id}
                          className="grid grid-cols-[60px_1.2fr_1.5fr_110px_100px] gap-3 items-center border-b border-slate-50 px-4 py-3 text-sm text-slate-700 hover:bg-slate-50/50 transition-colors"
                        >
                          <p className="font-medium">{user.id}</p>
                          <p className="font-medium text-slate-800">{user.username}</p>
                          <p className="text-slate-600">{user.displayName}</p>
                          <p className="capitalize text-xs font-medium text-slate-600 bg-slate-100 rounded px-2 py-1 inline-block w-fit">{user.role}</p>
                          <button
                            onClick={() => handleToggleUserActive(user)}
                            disabled={!canCreateUsers || usersUpdatingId === String(user.id)}
                            className={`ui-btn rounded-md px-2 py-1 text-xs font-medium transition-colors ${user.active
            ...
            {page === 'settings' && (
              <div className="rounded-xl border border-slate-200 bg-white p-6 max-w-2xl">
                <h3 className="mb-6 text-sm font-semibold text-slate-800">Configuration</h3>
                <div className="space-y-4">
                  ...
                  <label className="flex flex-col gap-2">
                    <span className="text-sm font-medium text-slate-700">Exchange Rate (1 USD = ? KHR)</span>
                    <input
                      type="number"
                      min={1}
                      step="100"
                      ...
                      onChange={(event) =>
                        setSettings((previous) => ({
                          ...previous,
                          exchangeRate: Number(event.target.value || 4000),
                        }))
                      }
                      className="ui-input px-3 py-2 text-sm"
                    />
                  ...
                  <label className="flex flex-col gap-2">
                    <span className="text-sm font-medium text-slate-700">Default Service</span>
                    <DropdownField
                  ...
                  <label className="flex flex-col gap-2">
                    ...
                    <button
                      ...
                      className={`ui-btn rounded-lg px-3 py-2 text-sm font-medium transition-colors ${settings.receiptFooter
                        ? 'bg-emerald-100 text-emerald-700 hover:bg-emerald-200'
                        : 'bg-slate-100 text-slate-600 hover:bg-slate-200'
                        }`}
                    >
