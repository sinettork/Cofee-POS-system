...
import { formatCurrency, convertCurrency } from '@shared/utils/format'
...
export function PaymentModal({
  ...
  // Calculate display amounts - these are shown to the customer during payment
  const totalInPaymentCurrency = paymentCurrency === currency
    ? totalAmount
    : convertCurrency(totalAmount, currency, paymentCurrency, exchangeRate)

  // Amount received is always in the payment currency
  const amountReceived = isCashPayment ? (isValidReceived ? parsedAmount : 0) : totalInPaymentCurrency
  const remainingAmount = Math.max(0, totalInPaymentCurrency - amountReceived)
            ...
            {METHODS.map((method) => {
              ...
              return (
                <button
                  ...
                  className={`flex flex-col items-center gap-3 rounded-[22px] border p-4 transition-colors ${active
                    ? 'border-[var(--ui-primary)]/30 bg-[#f7efe9] text-[var(--ui-primary)]'
                    : 'border-stone-100 bg-white text-stone-500 hover:border-stone-200 hover:bg-[#faf4ef]'
                    }`}
                >
                  <div className={`flex h-10 w-10 items-center justify-center rounded-full ${active ? 'bg-[var(--ui-primary)] text-white' : 'bg-stone-50 text-stone-400 ring-1 ring-stone-100'}`}>
                    <Icon size={18} />
                  </div>
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center p-3 md:p-5">
        ...
        <section className="ui-modal-card relative z-10 w-full max-w-[460px] p-4">
          <div className="mb-3 flex items-center justify-between">
            ...
            <button
              onClick={onClose}
              className="ui-btn ui-btn-ghost ui-icon-btn h-8 w-8 text-stone-400 hover:text-[var(--ui-primary)]"
            >
              <X size={15} />
            </button>
          </div>
          ...
          <div className="rounded-2xl border border-slate-200 bg-slate-50/70 px-4 py-4">
            ...
            <div className="mx-auto mt-4 w-full max-w-[320px]">
              <div className="flex items-center justify-center">
                <span
                  className={`rounded-full px-4 py-1.5 text-base font-black tabular-nums ${khqrCountdownSeconds > 30
    ...
    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center p-3 md:p-5">
        ...
        <section className="ui-modal-card relative z-10 w-full max-w-[420px] p-4">
          <div className="mb-3 flex items-center justify-between">
            <p className="text-xs font-semibold uppercase tracking-wide text-stone-500">Card Payment</p>
            <button
              onClick={onClose}
              className="ui-btn ui-btn-ghost ui-icon-btn h-8 w-8 text-stone-400 hover:text-[var(--ui-primary)]"
            >
          ...
          <div className="rounded-xl border border-stone-200 bg-stone-50/70 p-3">
            <p className="text-[11px] font-semibold uppercase tracking-wide text-stone-500">Amount Due</p>
            <p className="mt-1 text-2xl font-black text-[var(--ui-primary)]">
              {formatCurrency(totalAmount, currency)}
            </p>

            <div className="mt-3 rounded-xl border border-stone-200 bg-white p-3 text-sm text-stone-600">
              <p>Tap/insert card on payment terminal to process this amount.</p>

              {cardProcessing && (
                <p className="mt-2 inline-flex items-center gap-2 text-[var(--ui-primary)]">
        ...
        <div className="mb-6 grid grid-cols-3 gap-3">
          {METHODS.map((method) => (
            <button
              ...
              className={`flex flex-col items-center gap-2 rounded-[22px] border px-3 py-3 text-xs font-bold uppercase tracking-widest transition-colors ${selectedMethod === method.id
                ? 'border-[var(--ui-primary)]/30 bg-[#f7efe9] text-[var(--ui-primary)]'
                : 'border-stone-100 bg-white text-stone-500 hover:border-stone-200 hover:bg-[#faf4ef]'
                }`}
            >
              <span className={`inline-flex h-9 w-9 items-center justify-center rounded-full ${selectedMethod === method.id
                ? 'bg-[var(--ui-primary)] text-white'
                : 'bg-stone-50 text-stone-400 ring-1 ring-stone-100'
                }`}>
                {method.id === 'Cash' ? <Banknote size={16} /> : method.id === 'KHQR' ? <QrCode size={16} /> : <CreditCard size={16} />}
              </span>
              {method.label}
            </button>
          ))}
        ...
        <div className="rounded-3xl border border-stone-100 bg-white p-6 shadow-sm ring-1 ring-stone-100">
          ...
          <div className="grid grid-cols-2 gap-0 overflow-hidden rounded-2xl border border-stone-100 bg-stone-50/50">
            <div className="border-r border-stone-100 px-4 py-3">
              ...
              <p
                className={`tabular-nums text-2xl leading-tight font-black ${remainingAmount > 0 ? 'text-red-500' : 'text-stone-700'
                  }`}
              >
                {formatCurrency(remainingAmount, paymentCurrency)}
              </p>
            </div>
            <div className="px-4 py-3 text-right">
              ...
              <p className="tabular-nums text-2xl leading-tight font-black text-[#1C8370]">
                {formatCurrency(changeBackAmount, paymentCurrency)}
              </p>
            </div>
          </div>
        </div>
